...
*
* 당연히 정렬문제이다.
* 다만 문제는 탱크들의 이동을 출력해야 한다는 것이다.
* 탱크들의 이동순서에 따라 이동하지 못하는 경우가 생길 수 있다.
* x축인 좌우이동과 y축인 상하이동은 독립적으로 작동한다.
* 또한, 각 장난감 탱크마다 어떤 r로 가는지, 어떤 y로 가는지 정렬로 확인할 수 있다.
* 그러므로 x축(y축)이동에서 좌(상)으로 가는 경우 R(C)가 낮은 순으로 먼저 이동하고
* 우(하)로 가는 경우 R(C)가 높은 순으로 먼저 이동시키면 경로가 겹치거나 막힐 일이 없다.
*/
