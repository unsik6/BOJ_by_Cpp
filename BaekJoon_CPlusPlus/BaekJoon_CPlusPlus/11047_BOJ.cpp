...
// 만약 K >= Ai고 가치가 가장 큰 동전을 Am이라고 한다면, 최적해에 Am이 포함되지 않으면,
// 그 것보다 작은 동전들로 채웠을 때, Ai/Ak (k < i)만큼의 개수가 추가되므로
// Am을 선택했을 때 가장 최적인 해를 구할 수 있다.

// 최적 부분 구조
// K에 대해 Am을 최대한 넣고 남은 값을 K'라고 할 때 K'은 반드시 Am보다 작다.
// K' < Am이므로 Am은 더이상 사용할 수 없으며 Am > Ak (k < m)을 통해서 K'에 대한 최적해를 구한다.
// 이 때, K'에 대한 문제 즉 subProblem은 K일 때와 마찬가지로 구할 수 있으며 각각에 대한 최적해를 갖는다.
